...
theme, element_text, ggtitle, scale_y_continuous, geom_bar, ggsave, element_text, geom_density, \
after_stat
...
class OptionPricing_RL_move_GARCH:
    ...
    def fitted_lspi_put_option_iteration_GARCH(self, 
        ...
        wts: np.ndarray = np.ones(num_features)*10

        result_history = {}

        result_history[0] = LinearFunctionApprox.create(
                feature_functions=features, weights=Weights.create(wts)
            )
        
        #result_history = {}

        for iteration_number in range(1,training_iters):
    ...
    def clean_and_visualize_iteration_decision(self, stoptime_rl_iteration):
        ...
        g = (ggplot(df, aes(x='Values', fill='Iter', y= after_stat('width*density')))
             ...
             labs(y='Density', x="The Stopping Time Step (k)") +
...
option_pricing_rl_move = OptionPricing_RL_move_GARCH(num_scoring_paths_val=20000, 
